declare const handleId: unique symbol;
interface CurlHandle extends Record<any, any> {
    [handleId]: unknown;
}
export declare function curl_easy_init(): CurlHandle;
...
export type ProgressCallback = (this: CurlHandle, dltotal: number, dlnow: number, ultotal: number, ulnow: number) => number;
export type SeekCallback = (this: CurlHandle, ossfer: number, origin: number) => number;
export type PreformCallback = (this: CurlHandle, resultCode: CurlEConstants[keyof CurlEConstants], resultString: string) => void;
export declare function curl_easy_perform(curl: CurlHandle, callback: PreformCallback): void;
export declare function curl_easy_pause(curl: CurlHandle, bitmask: number): void;
export declare function curl_easy_headers(curl: CurlHandle, origin: number, request: number): HttpHeaders;
export declare function curl_easy_cleanup(curl: CurlHandle): void;
export declare function curl_easy_reset(curl: CurlHandle): void;
...
export declare function curl_easy_getinfo<T extends keyof CurlInfoToType>(curl: CurlHandle, info: CurlInfoConstants[T]): CurlInfoToType[T];
...
export declare function curl_ws_recv(curl: CurlHandle, buffer: ArrayBuffer | Uint8Array): CurlRecvResult;
interface CurlSendResult {
    result: CurlEConstants[keyof CurlEConstants];
    sent: number;
}
export declare function curl_ws_send(curl: CurlHandle, buffer: ArrayBuffer | Uint8Array, fragsize: number, flags: number): CurlSendResult;
export declare function update(): void;
export declare const constants: CurlEConstants & CurlConstants & CurlInfoConstants & CurlOptConstants;
